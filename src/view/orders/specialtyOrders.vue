<template>
    <div id="pageContainer" ref="container">
        <div class="container-pos">
            <mu-load-more @refresh="refresh" :refreshing="refreshing" :loading="loading1" @load="load">
                <div class="list" v-if="hasmore != 0">
                    <router-link :to="'/orders/specialty?id=' + item.specialtyOrderId" class="item" v-for="item in list" :key="item.specialtyOrderId">
                        ...
                        <div class="other flex spb fcen">
                            <p class="txt">共{{item.count}}件</p>
                            <div class="price flex fend">
                                <span>¥</span>
                                <span class="num">{{item.totalPrice}}</span>
                            </div>
                        </div>
                    </router-link>
        ...
        <div class="menus flex">
            <router-link to="/specialty" class="menu fcol fcen spc">
                <img src="../../assets/img/tc/tc1.png" alt="" />
                <span>特产</span>
            </router-link>
            <Kefu></Kefu>
            <a href="javascript:;" class="menu fcol fcen spc on">
                <img src="../../assets/img/tc/order2.png" alt="" />
                <span>订单</span>
            </a>
        </div>
    </div>
...
import { specialtyOrderList } from '../../api/specialty';
...
export default {
    ...
    methods: {
        getData() {
            ...
            specialtyOrderList({ pageIndex: this.pageIndex, pageSize: this.pageSize }).then(res => {
